...
export const systemPrompt = ({ userMessageId, resumeContextForChat }: { userMessageId: string, resumeContextForChat: YooptaContentValue | any }) => {
  ...
  prompt += "- **PRIORIDADE DE CONTEXTO:** Para TODAS as tarefas (sugestões, comandos de edição, perguntas sobre o estado atual), SEMPRE use o 'Contexto Atual do Currículo' acima como a fonte primária de informação. Se um PDF foi analisado anteriormente, considere essa análise como inicial, mas o JSON acima representa o estado ATUAL e mais importante.\n";
  prompt += "- **DISCREPÂNCIA PDF vs CONTEXTO:** Se você analisar um arquivo anexado (PDF) e notar que ele representa um currículo diferente do 'Contexto Atual do Currículo' acima (ex: nomes diferentes), sua PRIMEIRA resposta DEVE:\n"
  prompt += "  1. Apontar CLARAMENTE a discrepância (ex: 'O currículo no editor é para [Nome no Contexto], mas o arquivo é para [Nome no PDF].').\n"
  prompt += "  2. Perguntar ao usuário qual contexto ele deseja usar (o do editor ou o do arquivo PDF).\n"
  prompt += "  3. Se você conseguiu extrair informações chave do PDF (pelo menos o nome), inclua-as formatadas entre marcadores na MESMA resposta, APÓS a pergunta: [[PDF_INFO_START]]{ \"name\": \"Nome Extraído do PDF\", \"title\": \"Título Extraído (opcional)\" }[[PDF_INFO_END]]\n";
